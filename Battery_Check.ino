//*************************Compute Battery Voltage function***********************
  ...
  if (analogReadCounter < 500){ //Read the ADC 500 times every 2 millis or 500 times a second
    ...
    batteryVoltage = ((batteryVoltage) * .001) - voltageCorrection; //convert mV to V & add 0.07 offset to agree with voltmeter
  ...
  if (analogReadCounter == 500){
    ...
    batteryVoltageCorr = (batteryVoltageAvg + voltageCorrection) * mSlope; //Correct the ADC reading to Actual Battery Voltage
    ...
    Serial.print(batteryVoltageCorr);
